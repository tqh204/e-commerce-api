...
namespace Infrastructure.Repositories
{
    public class VariantRepository : IVariantRepository
    {
        ...
        public async Task<Variant?> GetByIdAsync(Guid variantId) =>
        ...
        public async Task<bool> ExistsSkuAsync(string sku, Guid? excludeVariantId = null) =>
            await _context.Variants.AnyAsync(v =>
                !v.isDeleted &&
                v.sku == sku &&
                (!excludeVariantId.HasValue || v.variantId != excludeVariantId.Value));
        public async Task<bool> ProductExistsAsync(Guid productId) =>
            await _context.Products.AnyAsync(p => !p.isDeleted && p.productId == productId);
