...
namespace Infrastructure.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            var query = _context.Products.Include(u => u.category).Include(u => u.variants).Where(u => !u.isDeleted).AsQueryable();//Starting at Product table and taking categories atrributes associated and used where to get product which is active. Finally AsQueryable to use connect to another syntax
        ...
        public async Task<Product?> GetProductByIdAsync(Guid productId) => await _context.Products.Include(x => x.category).Include(x => x.variants).Where(u => !u.isDeleted).FirstOrDefaultAsync(u => u.productId == productId);
